...
## 方案
基于以上分析，提出以下方案来对比开启/不开启async-profiler时的CPU消耗，latency，内存占用等：
1. 基准测试设置：
    - 编写一个web程序作为基准测试，其内部通过执行序列化达到较高CPU消耗，较高耗时，内存占用高以增强对比。
    - 在不启用async-profiler的情况下，运行基准测试多次，记录平均CPU利用率、响应时间和内存消耗作为基准数据。
2. 启用async-profiler：
    - 使用配置参数启动async-profiler，分别设置仅采样CPU，仅采样内存等。
    - 采样间隔设为较大的值以减少影响。
3. 对比数据：
